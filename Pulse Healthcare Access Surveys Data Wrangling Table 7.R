...
wd <- "D:\\Data sets and Coding Files\\Household Pulse Surveys Healthcare Access\\Table7"
setwd(wd)
getwd()
...
dt$week <- as.numeric(str_remove(str_remove(dt$file, "health7_week"), ".xlsx"))
dt <- dt %>% arrange(week)
...
names(data) <- c("demog", "prev_covid_diag_Y", "prev_covid_diag_N", "prev_covid_diag_DNR", "dnr",  "week")
...
write.csv(data_final, "D:\\Data sets and Coding Files\\Household Pulse Surveys Healthcare Access\\HPS_HA_T7_Wrang.csv", row.names = FALSE)
