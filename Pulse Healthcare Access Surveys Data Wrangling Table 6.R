...
wd <- "D:\\Data sets and Coding Files\\Household Pulse Surveys Healthcare Access\\Table6"
setwd(wd)
getwd()
...
dt$week <- as.numeric(str_remove(str_remove(dt$file, "health6_week"), ".xlsx"))
dt <- dt %>% arrange(week)
...
names(data) <- c("demog", "concern_side_effs", "concern_if_works", "dont_need_vax", "dont_like_vax",
                 "doctor_not_rec","wait_to_see", "others_need", "cost", "dont_trust_covid_vax",
                 "dont_trust_govt", "other", "dnr", "week")
...
write.csv(data_final, "D:\\Data sets and Coding Files\\Household Pulse Surveys Healthcare Access\\HPS_HA_T6_Wrang.csv", row.names = FALSE)
