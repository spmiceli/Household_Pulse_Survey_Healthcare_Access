...
wd <- "D:\\Data sets and Coding Files\\Household Pulse Surveys Healthcare Access\\Table2b"
setwd(wd)
getwd()
...
dt$week <- as.numeric(str_remove(str_remove(dt$file, "health2b_week"), ".xlsx"))
dt <- dt %>% arrange(week)
...
dt_2$`Not at all...2` <- coalesce(dt_2$`Not at all...2`, 
                                  as.double(dt_2$`Frequency of having little interest or pleasure in doing things...2`))
dt_2$`Several days...3` <- coalesce(dt_2$`Several days...3`, 
                                    as.double(dt_2$`Frequency of having little interest or pleasure in doing things...3`))
dt_2$`More than half the days...4` <- coalesce(dt_2$`More than half the days...4`, 
                                               as.double(dt_2$`Frequency of having little interest or pleasure in doing things...4`))
dt_2$`Nearly everyday...5` <- coalesce(dt_2$`Nearly everyday...5`, 
                                       as.double(dt_2$`Frequency of having little interest or pleasure in doing things...5`))
dt_2$`Did not report...6` <- coalesce(dt_2$`Did not report...6`, 
                                      as.double(dt_2$`Frequency of having little interest or pleasure in doing things...6`))
dt_2$`Not at all...7` <- coalesce(dt_2$`Not at all...7`, 
                                  as.double(dt_2$`Frequency of feeling down, depressed, or hopeless...7`))
dt_2$`Several days...8` <- coalesce(dt_2$`Several days...8`, 
                                    as.double(dt_2$`Frequency of feeling down, depressed, or hopeless...8`))
dt_2$`More than half the days...9` <- coalesce(dt_2$`More than half the days...9`, 
                                               as.double(dt_2$`Frequency of feeling down, depressed, or hopeless...9`))
dt_2$`Nearly everyday...10` <- coalesce(dt_2$`Nearly everyday...10`, 
                                        as.double(dt_2$`Frequency of feeling down, depressed, or hopeless...10`))
dt_2$`Did not report...11` <- coalesce(dt_2$`Did not report...11`, 
                                       as.double(dt_2$`Frequency of feeling down, depressed, or hopeless...11`))
...
write.csv(data_final, "D:\\Data sets and Coding Files\\Household Pulse Surveys Healthcare Access\\HPS_HA_T2b_Wrang.csv", row.names = FALSE)
